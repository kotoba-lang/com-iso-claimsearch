(ns iso-claimsearch.main
  "Kotodama WASM entrypoint for the iso_claimsearch clean-room actor (L4) — Clojure port.
  ...
  facts (`iso_claimsearch.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "iso_claimsearch")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"      :plural "accounts"      :id-prefix "isoclaim_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                 :refs {}}
   {:entity "Loan"         :plural "loans"         :id-prefix "isoclaim_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"iso_claimsearch.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iso_claimsearch-compat" :tier tier :entities entities} 200])
...
(defn start! [] :iso_claimsearch-compat/ready)
